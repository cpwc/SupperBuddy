var Order = function () {
	...
	var handleFoodSelect = function() {
		...
		$("select#testasdf").change(function() {
			...
			$.ajax({
				type: "GET",
				url: "http://localhost/index.php/food/detail/" + $food_id,
				success: function (data)
				{
	...
	var handleSubOrderAdd = function() {
		...
		function editRow(oTable, nRow) {
			var aData = oTable.fnGetData(nRow);
			var jqTds = $('>td', nRow);
			jqTds[0].innerHTML = '<input type="text" class="form-control input-small" value="' + nRow.rowIndex + '" readonly>';
			jqTds[1].innerHTML = '<select id="testasdf" class="form-control input-large select2me" data-placeholder="Select..."><option value=""></option></select>';
			jqTds[2].innerHTML = '<input id="price" type="text" class="form-control input-small" value="' + aData[2] + '" readonly>';
			jqTds[3].innerHTML = '<input id="quantity" type="text" class="form-control input-small" value="' + aData[3] + '">';
		...
		function saveRow(oTable, nRow) {
			...
			var txt = $("option:selected", jqInputs[2]).text();
			oTable.fnUpdate(jqInputs[0].value, nRow, 0, false);
			oTable.fnUpdate(txt, nRow, 1, false);
		...
		function populateSelect() {
			...
			$.ajax({
				type: "GET",
				url: "http://localhost/index.php/food/list_caterer/" + $caterer_id,
				success: function (data)
				{
					var sel = $("#testasdf");
					sel.empty();
